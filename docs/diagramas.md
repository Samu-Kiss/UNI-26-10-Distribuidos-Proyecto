...
namespace common.mensajes {
    ...
    class SnapshotOperativo:::modelos {
        ...
        +vehiculos: list[dict]
        +a_dict() dict
        +crear(timestamp, tick_actual, intersecciones, vias, vehiculos, fuente, version_contrato) SnapshotOperativo
...
loop por cada via instrumentada y tipo_sensor
    PC1Sens->>PC1Sens: construir_datos(tipo_sensor, via, intervalo_espira_segundos)
